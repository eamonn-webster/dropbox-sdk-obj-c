...
- (void)displayImage:(NSData*) fileData {
    CGFloat roomForButton = 50;
    CGFloat padding = 10;
    CGFloat w = NSWidth([self.view bounds]);
    CGFloat h = NSHeight([self.view bounds]);
    NSImageView *imageView = [[NSImageView alloc] initWithFrame:NSMakeRect(0, 0, w - padding, (h - roomForButton) - padding)];
  [imageView setImage:[[NSImage alloc] initWithData:fileData]];
  [imageView setFrameOrigin:NSMakePoint(w/2, h/2)];
    CGFloat x = (w - NSWidth([imageView frame])) / 2;
    CGFloat y = ((h - roomForButton) - NSHeight([imageView frame])) / 2 + roomForButton;
  // NSLog(@"bounds: %f image %f x %f y %f",  NSHeight([self.view bounds]), NSHeight([imageView frame]), x, y);
  ...
  self->_currentImageView = imageView;
  [self setFinished];
}
